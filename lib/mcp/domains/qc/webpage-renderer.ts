...
export class WebpageRenderer {
  static renderToHtml(article: MdxArticle): string {
    ...
    let bodyHtml = article.content
      ...
      .replace(/!\[(.*?)\]\((.*?)\)/gim, '<figure class="article-figure"><img src="$2" alt="$1" style="max-width:100%; height:auto;" /><figcaption>$1</figcaption></figure>')
      ...
      .map((p) => {
        ...
        if (trimmed.startsWith('<h') || trimmed.startsWith('<figure') || trimmed.startsWith('<blockquote>') || trimmed.startsWith('<li>')) {
          return trimmed
        }
