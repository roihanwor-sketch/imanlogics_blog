  ...
  if (fs.existsSync(LOCK_FILE)) {
    try {
      ...
      try {
        ...
        console.log(
          `ℹ️ [Scheduler Daemon] Another instance is already running (PID: ${lockData.pid}, Started: ${lockData.startedAt}). Exiting to prevent duplication.`
        )
        return false
      } catch {
        // Stale lock file
