...
import {
  researchIslamicAcademicIntelligence,
  IslamicAcademicStory,
} from './islamic-logic-researcher'
import {
...
export async function runAutonomousEditorialPipeline(
  ...
  console.log(
    `📊 [Pipeline Audit] Discovered ${report.candidatesDiscovered} candidate story/stories across ${report.sourcesScanned} primary/secondary sources.`
  )
  ...
  if (totalSelected === 0) {
    console.log(
      `ℹ️ [Editorial Decision] NO_PUBLISHABLE_STORY — No fresh stories met strict news hook or anti-duplicate criteria.`
    )
  ...
  for (const story of selectedTech) {
    ...
    if (allPassed) {
      for (const article of articles) {
        ...
        console.log(
          `  💾 Published MDX: data/blog/${article.filename} [QC: ${qcResults[article.language].score}/100]`
        )
      }
  ...
  if (options.gitPush || process.env.CI) {
    try {
      ...
      execSync(
        `git commit -m "feat(blog): autonomous publication of ${publishedArticles.length} trilingual articles [QC >= 85]"`
      )
